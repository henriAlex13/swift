...
class SwiftMatcher:
    ...
    def __init__(self, mt910_dir: str, pacs008_dir: str, match_dir: str = "MATCH", 
        ...
        # Date de début du traitement (format: MMAAAA, ex: 082025 pour août 2025)
    ...
    def _parse_start_date(self, start_date: str) -> datetime:
        """
        Parse la date de début au format MMAAAA (ex: 082024)
        ...
        try:
            ...
            logging.warning(f"⚠️  Format de date invalide: {start_date}, utilisation d'août 2024 par défaut")
            return datetime(2024, 8, 1)
    
    def _is_date_in_range(self, month_str: str) -> bool:
        """
        Vérifie si un mois_annee (ex: 0824) est dans la plage de traitement
        """
        try:
            ...
            # En cas d'erreur de parsing, on traite le fichier par sécurité
            return True
    ...
    def scan_directory(self, base_dir: Path, message_type: str, full_scan: bool = False) -> List[SwiftMessage]:
        ...
        for month_dir in base_scan_dir.iterdir():
            ...
            # Vérifier que c'est bien un répertoire mois_annee (ex: 0125)
    ...
    def _extract_date_structure(self, file_path: str) -> Tuple[str, str]:
        ...
        Exemples: 
        - /path/to/swift_sgci/0125/010125/entrant/mt910/file.pdf -> ('0125', '010125')
        - /path/to/0125/010125/entrant/pacs.008/manu/sgci/file.pdf -> ('0125', '010125')
        """
        path_parts = Path(file_path).parts
        
        # Chercher les patterns mois_annee (4 chiffres) et jour_mois_annee (6 chiffres)
        month_dir = None
        day_dir = None
        
        for i, part in enumerate(path_parts):
            # Chercher mois_annee (format: 0125)
            if re.match(r'^\d{4}
...
, month_dir.name):
            ...
            for day_dir in month_dir.iterdir():
                ...
                # Vérifier que c'est bien un répertoire jour_mois_annee (ex: 010125)
...
, day_dir.name):
                ...
                # Si le chemin n'existe pas, passer au suivant
                if not target_dir.exists():
                    continue
...
if __name__ == "__main__":
    ...
    stats, matches, mt910_unmatched, pacs008_unmatched = matcher.run_matching(OUTPUT_DIR)
, part) and month_dir is None:
                month_dir = part
                # Le jour devrait être le prochain répertoire
                if i + 1 < len(path_parts) and re.match(r'^\d{6}
